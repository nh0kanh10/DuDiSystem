  ...
  return (
    <div className="space-y-6 select-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard title="Tổng nhân sự" value={stats?.totalEmployees || 0} sub="Đang hoạt động trong hệ thống" iconBg="bg-blue-50/50" iconColor="text-blue-600" icon={Users} />
        <StatCard title="Đúng giờ / Trễ" value={`${stats?.attendance?.onTime || 0} / ${stats?.attendance?.late || 0}`} sub="Dữ liệu chấm công hôm nay" iconBg="bg-green-50/50" iconColor="text-green-600" icon={UserCheck} />
        <StatCard title="Vắng mặt" value={stats?.attendance?.absent || 0} sub="Nghỉ không phép hôm nay" iconBg="bg-red-50/50" iconColor="text-red-600" icon={UserX} />
        <StatCard title="Nghỉ phép" value={stats?.attendance?.leave || 0} sub="Đăng ký nghỉ có phép hôm nay" iconBg="bg-purple-50/50" iconColor="text-purple-600" icon={Clock} />
      </div>
