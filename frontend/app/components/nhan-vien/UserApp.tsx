...
function ModalWrapper({ children, onClose, title, subtitle }: { children: React.ReactNode; onClose: () => void; title: string; subtitle?: string; }) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[1000] flex items-center justify-center pb-[6vh] bg-black/60 dark:bg-black/40 backdrop-blur-md transition-opacity duration-[240ms] ease-out ${visible ? "opacity-100" : "opacity-0"}`}
      onClick={handleClose}
    >
      <div
...
// Panel cho Tasks và Checkin (full-page)
function Panel({
  activePage,
  onClose,
  employee,
...
function DirectoryContent() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2.5">
        {list.map((emp) => (
          <div
            ...
            className="flex items-center gap-3.5 p-3.5 bg-white dark:bg-white/[0.04] border border-[#efd7da] dark:border-white/10 rounded-2xl shadow-[0_12px_30px_rgba(95,15,22,0.06)] dark:shadow-none cursor-pointer hover:border-[#D4472A]/50 transition-all"
          >
            <div
      ...
      {selectedEmp && (
        <div
          className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/20 dark:bg-black/60 backdrop-blur-[12px] transition-opacity"
      ...
      <div className="flex flex-col gap-4 bg-transparent w-full">
        ...
        <div className="flex items-center gap-[18px] px-5 py-4 bg-white dark:bg-white/[0.04] border border-[#efd7da] dark:border-white/10 rounded-[18px] shadow-[0_14px_36px_rgba(95,15,22,0.07)] dark:shadow-none">
          <div style={{ position: "relative", flexShrink: 0 }}>
            <div className="w-[68px] h-[68px] rounded-full flex items-center justify-center text-[20px] font-black text-white bg-gradient-to-br from-[#E8231A] to-[#FF8800] dark:from-[#EF4444] dark:to-[#EF4444]/70 shadow-[0_0_24px_rgba(232,35,26,0.28)] dark:shadow-[0_0_20px_rgba(239,68,68,0.6)]">
        ...
        <div className="flex items-center border-b border-[rgba(36,20,22,0.1)] dark:border-white/10 mt-0.5">
          {["Thông tin chung", "Công việc", "Liên hệ", "Tài liệu & Hình ảnh", "Lịch sử"].map((t, i) => {
            const badges: Record<number, number> = { 3: 4, 4: 4 };
        ...
        {activeTab === 3 && (
          <div className="p-[16px_18px] bg-white dark:bg-white/[0.04] border border-[#efd7da] dark:border-white/10 rounded-2xl">
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {["CV_NguyenVanA.pdf", "CCCD_Mat_Truoc.jpg", "CCCD_Mat_Sau.jpg", "Bang_Tot_Nghiep.pdf"].map((file, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-[12px_14px] bg-[#fff5f5]/70 dark:bg-white/[0.04] border border-[#e8231a]/10 dark:border-white/10 rounded-xl"
                >
                  <div className="flex items-center gap-[10px]">
                    <FileText size={20} className="text-[#E8231A] dark:text-red-400" />
                    <span className="text-[14px] font-[600] text-[#241416] dark:text-gray-200">
                      {file}
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <button className="px-3 py-1.5 bg-[#E8231A] dark:bg-red-500/20 text-white dark:text-red-300 border-none rounded-lg cursor-pointer text-[12px] font-[700]">
                      Xem
                    </button>
                    <button className="px-3 py-1.5 bg-white dark:bg-transparent text-[#E8231A] dark:text-red-400 border border-[#E8231A] dark:border-red-400/50 rounded-lg cursor-pointer text-[12px] font-[700]">
                      Tải
                    </button>
                  </div>
                </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {history.map((entry: any, idx: number) => (
                  <div key={entry.id} className="flex items-start gap-2.5">
                    ...
                    />
                    <div>
                      <p className={`text-[14px] ${idx === 0 ? "font-[800] text-[#241416] dark:text-gray-100" : "font-[700] text-[#5f4246] dark:text-gray-400"}`}>
                        {entry.title}
                      </p>
                      <p className="text-[12px] text-[#7f5f63] dark:text-gray-500 mt-[2px]">
                        {[
                          entry.snapshot,
                          entry.date,
    ...
    <div style={{ width: "100%", height: embed ? "100%" : "100vh", overflow: "hidden", position: embed ? "absolute" : "relative", inset: embed ? 0 : undefined }}>
      ...
      {!employee ? (
        <div style={{
          ...
          <div style={{ fontSize: 14, opacity: 0.7 }}>Vui lòng đợi trong giây lát</div>
        </div>
      ) : (
        <>
          {/* Main Interface: GreetingMessageApp with fallback */}
      ...
      {activePage === "crm" && employee && (
        <ModalWrapper title="Quản lý khách hàng" onClose={() => setActivePage(null)}>
      ...
      {/* Tasks và Checkin dùng Panel (full-page) */}
      {(activePage === "tasks" || activePage === "checkin") && employee && (
        <Panel
