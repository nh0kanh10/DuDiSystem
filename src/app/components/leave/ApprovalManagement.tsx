...
const TIMEOFF_SLOTS: TimeOffSlot[] = [
  { id: "S1", empId: "NV003", empName: "Lê Thu Hương", empCode: "2023081567", department: "Design", day: 1, session: "sang", reason: "Dạ đi học", status: "approved", week: "W26", registeredAt: "22:17:10 21/6/2026", adminNote: "Đã được duyệt tự động (đồng bộ)", processedAt: "22:17:10 21/6/2026" },
  { id: "S2", empId: "NV003", empName: "Lê Thu Hương", empCode: "2023081567", department: "Design", day: 1, session: "chieu", reason: "Dạ đi học", status: "approved", week: "W26", registeredAt: "22:17:10 21/6/2026", adminNote: "Đã được duyệt tự động (đồng bộ)", processedAt: "22:17:10 21/6/2026" },
  { id: "S3", empId: "NV004", empName: "Phạm Đức Thành", empCode: "2025012893", department: "PM", day: 2, session: "sang", reason: "Họp khách hàng", status: "pending", week: "W26", registeredAt: "08:00:00 22/6/2026", adminNote: "", processedAt: "" },
  { id: "S4", empId: "NV002", empName: "Nguyễn Văn Minh", empCode: "2024031245", department: "Backend", day: 2, session: "chieu", reason: "Việc cá nhân", status: "pending", week: "W26", registeredAt: "07:55:00 22/6/2026", adminNote: "", processedAt: "" },
  { id: "S5", empId: "NV005", empName: "Hoàng Thị Mai", empCode: "2024091234", department: "HR", day: 3, session: "sang", reason: "Đi khám bệnh", status: "approved", week: "W26", registeredAt: "18:00:00 20/6/2026", adminNote: "Duyệt", processedAt: "09:00:00 21/6/2026" },
  { id: "S6", empId: "NV006", empName: "Võ Minh Tuấn", empCode: "2025060001", department: "Backend", day: 3, session: "chieu", reason: "Bù tăng ca", status: "approved", week: "W26", registeredAt: "10:00:00 21/6/2026", adminNote: "Duyệt bù giờ", processedAt: "10:05:00 21/6/2026" },
  { id: "S7", empId: "NV007", empName: "Đinh Thị Lan Anh", empCode: "2025071501", department: "Frontend", day: 4, session: "sang", reason: "Gia đình có việc", status: "rejected", week: "W26", registeredAt: "09:30:00 22/6/2026", adminNote: "Không đủ điều kiện", processedAt: "10:00:00 22/6/2026" },
  { id: "S8", empId: "NV001", empName: "Trần Thị Bích Liên", empCode: "2026052831", department: "Frontend", day: 4, session: "chieu", reason: "Bù tăng ca T5", status: "pending", week: "W26", registeredAt: "22:00:00 22/6/2026", adminNote: "", processedAt: "" },
  { id: "S9", empId: "NV004", empName: "Phạm Đức Thành", empCode: "2025012893", department: "PM", day: 5, session: "sang", reason: "Học thêm", status: "approved", week: "W26", registeredAt: "07:00:00 20/6/2026", adminNote: "OK", processedAt: "08:00:00 20/6/2026" },
  { id: "S10", empId: "NV008", empName: "Bùi Văn Hùng", empCode: "2024112001", department: "DevOps", day: 5, session: "chieu", reason: "Trực ca đêm bù", status: "approved", week: "W26", registeredAt: "14:00:00 21/6/2026", adminNote: "Duyệt bù trực", processedAt: "15:00:00 21/6/2026" },
  { id: "S11", empId: "NV002", empName: "Nguyễn Văn Minh", empCode: "2024031245", department: "Backend", day: 6, session: "sang", reason: "Nghỉ phép năm", status: "pending", week: "W26", registeredAt: "08:00:00 23/6/2026", adminNote: "", processedAt: "" },
]
...
export default function ApprovalManagement() {
  ...
  const [slots, setSlots] = useState<TimeOffSlot[]>(TIMEOFF_SLOTS)
  ...
  // Quick operations directly on Grid cells
  const handleQuickApprove = (id: string) => {
    ...
    showToast("Đã duyệt đơn nghỉ phép thành công!")
  }
  ...
  const handleQuickReject = (id: string) => {
    ...
    showToast("Đã từ chối đơn nghỉ phép thành công!")
  }
  ...
  const handleApproveAllPending = () => {
    ...
    if (pendingSlots.length === 0) {
      showToast("Không có đơn nghỉ phép nào đang chờ duyệt!")
      return
    }
    setSlots(p => p.map(s => (s.status === "pending" && s.week === weekFilter) ? { ...s, status: "approved" as const, adminNote: "Duyệt nhanh hàng loạt", processedAt: new Date().toLocaleString("vi") } : s))
    showToast(`Đã phê duyệt nhanh tất cả (${pendingSlots.length}) đơn chờ duyệt!`)
  }
  ...
  const handleBulkReject = () => {
    ...
    showToast(`Đã từ chối hàng loạt thành công ${selectedSlotIds.length} đơn!`)
    setSelectedSlotIds([])
    setBulkMode(false)
  }
  ...
  const handleSaveLeaveType = () => {
    if (!newType.name.trim() || !newType.code.trim()) {
      showToast("Vui lòng nhập đầy đủ thông tin loại phép!")
      return
    }
    const colorClasses = [
  ...
  // Quick register slot action
  const handleCreateQuickSlot = () => {
    if (!quickAddReason.trim()) {
      showToast("Vui lòng nhập lý do nghỉ phép!")
      return
    }
    const newSlotId = `S${Date.now()}`
    const emp = TIMEOFF_SLOTS.find(s => s.empId === quickAddSlot?.empId)
    const newSlot: TimeOffSlot = {
      id: newSlotId,
      empId: quickAddSlot!.empId,
      empName: quickAddSlot!.empName,
      empCode: emp ? emp.empCode : "2026xxxx",
      department: emp ? emp.department : "Phòng ban",
      day: quickAddSlot!.day,
      session: quickAddSlot!.session,
      reason: quickAddReason,
      status: "approved" as const, // Duyệt thẳng luôn cho nhanh
      week: weekFilter,
  ...
  const DAYS = [
    { label: "Thứ 2", date: "23/06", day: 1 },
    { label: "Thứ 3", date: "24/06", day: 2 },
    { label: "Thứ 4", date: "25/06", day: 3 },
    { label: "Thứ 5", date: "26/06", day: 4 },
    { label: "Thứ 6", date: "27/06", day: 5 },
    { label: "Thứ 7", date: "28/06", day: 6 },
  ]

  const empIds = Array.from(new Set(slots.map(s => s.empId)))
  const empRows = empIds
    .map(id => slots.find(s => s.empId === id)!)
    .filter(e => e.empName.toLowerCase().includes(searchEmp.toLowerCase()) || e.department.toLowerCase().includes(searchEmp.toLowerCase()))
                              ...
                              {sang ? (
                                <div className="relative">
                                  ...
                                  {!bulkMode && sang.status === "pending" && (
                                    <div className="absolute inset-0 bg-white/80 backdrop-blur-[1px] rounded-xl flex items-center justify-center gap-1 opacity-0 group-hover/slot:opacity-100 transition-all z-10 duration-200">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/5">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                ...
                <Tooltip formatter={(v: any) => [`${v} lượt`, ""]} contentStyle={{ borderRadius: 12, border: "none", boxShadow: "0 4px 24px rgba(0,0,0,.08)" }} />
